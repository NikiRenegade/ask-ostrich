...
namespace SurveyResponseService.Presentation.API.Controllers
{
    ...
    public class SurveyResultController : ControllerBase
    {
        ...
        [HttpGet("survey/{surveyId}/guest/{guestId}")]
        public async Task<ActionResult<SurveyResultDto>> GetLatestBySurveyIdAndGuestId(Guid surveyId, Guid guestId, CancellationToken cancellationToken = default)
        {
            try
            {
                var result = await _surveyResultService.GetLatestBySurveyIdAndGuestIdAsync(surveyId, guestId, cancellationToken);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<IList<SurveyResultDto>>> GetBySurveyId(Guid surveyId, CancellationToken cancellationToken = default)
        {
            try
            {
                var results = await _surveyResultService.GetBySurveyIdAsync(surveyId, cancellationToken);
                return Ok(results);
            }
            catch (Exception ex)
            {
